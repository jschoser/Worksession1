...
class TriMesh(object):
  ...
  y1         =  1;                    # inner radius
  y2         =  11;                   # outer radius
  bnx        =  15;                   # Background elements in x
  bny        =  15;                   # Background elements in y
  ...
  def loadMesh(self,n): 
    ...
    for i in range(self.bnx+1):
      yvert.append(np.linspace(ylow[i], ytop[i], self.bny+1))
...
#   plt.set_ylabel('y',size=14,weight='bold')
  ...
  #=========================================================
  def plotRawSoln(self, solVec, title=""):
    ...
    plt.axes().set_aspect('equal', 'datalim')
    xy = np.asarray(self.mesh.vertices);
    plt.triplot(xy[:,0],xy[:,1],self.mesh.elements,'w-',linewidth=0.1);
...
#    plt.savefig('sol.pdf',dpi=250)
  ...
  #=========================================================
  # Makes a contour plot of the solution with satellites
  #=========================================================
  def plotSatSoln(self, solVec, xmS, ymS, xmQ, ymQ, title=""):
...
#    plt.imshow(plt.imread(r'topo_europa.png'), interpolation='nearest', extent=[-0.5,1.495,0,1], alpha=1.0)                            
    plt.triplot(xy[:,0],xy[:,1],self.mesh.elements,'w-',linewidth=0.1);
    plt.tricontourf(self.mesh.dtri,solVec, alpha=0.8, cmap=cmap)
    plt.plot(xmS, ymS, 'ro', xmQ, ymQ, 'bs')
    plt.colorbar()
#    plt.savefig('sol.pdf',dpi=250)
    plt.show()
    plt.tight_layout()
  ...
  def findMinimumPotential(self, solvec):
      xy = np.asarray(self.mesh.vertices);
      loc = np.where(solvec == solvec.min())	
