...
print ("n=",n," a,b=( ",a," ",b," )")
print ("")
print ("Perturbation potential at point Q = ",umQ);
print ("Perturbation potential at point S = ",umS);
print ("")
print ("Deviation from measurement: dev=", dev) 
print ("----------------------------------------\n")
...
fes.plotSoln(solVec, xmS, ymS, xmQ, ymQ, "Solution")
xIceWat = np.array(fes.lowerCoords)[:,0]
yIceWat=[]
for i in range(xIceWat.size):
    yIceWat.append(TriFEMLibGF.iceWat(xIceWat[i],a,b))
plt.plot(xIceWat,yIceWat,'k')
plt.plot(xIceWat,np.ones(xIceWat.size)*2, 'k')
plt.tight_layout()
plt.show()
